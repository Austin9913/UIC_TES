...
def _cfg(url='https://github.com/ggjy/CMT.pytorch/releases/download/release-v1/cmt_small.pth', **kwargs):
    return {
...
class CMT(nn.Module):
    def __init__(self, img_size=224, in_chans=3, num_classes=3, embed_dims=[46, 92, 184, 368], stem_channel=16,
                 fc_dim=1280,
...
def _create_cmt_model(pretrained=False, distilled=False, **kwargs):
    ...
    model = CMT(img_size=img_size, representation_size=repr_size, **kwargs)
    model.default_cfg = default_cfg

    # if pretrained:
    #     load_pretrained(model, num_classes=num_classes, in_chans=kwargs.get('in_chans', 3),
    #         filter_fn=partial(checkpoint_filter_fn, model=model))

    return model


def cmt_ti_fasd(pretrained=False, **kwargs):
    """
    CMT-Tiny
    """
