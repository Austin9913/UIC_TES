...
def main():
    ...
    # 创建数据集
    def create_dataset(func, n_samples=100, n_var=2):
        x = torch.normal(0, 1, size=(n_samples, n_var))
        y = func(x)
        return {'train_input': x[:80], 'train_label': y[:80], 'test_input': x[80:], 'test_label': y[80:]}

    # 获取数据集
    dataset = create_dataset(test_function)

    # 训练模型
    results = model.train(dataset, opt='LBFGS', steps=100, log=10, lamb=0.01, update_grid=False, save_fig=True, img_folder='./kan_figs')

    # 测试模型性能
    test_loss = torch.nn.MSELoss()(model(dataset['test_input']), dataset['test_label'])
    ...
    # 保存模型检查点（如果需要）
    # model.save_ckpt('kan_model_checkpoint')

    # 加载模型检查点（如果需要）
    # model.load_ckpt('kan_model_checkpoint')
