...
def main_test():
    ...
    # 创建模型并传到设备
    model = alt_gvt_small(pretrained=True, img_size=256, num_classes=100, drop_path_rate=0.1)

    model.to(device)

    # 随机生成输入的 tensor，假设输入大小是 (batch_size, 3, 224, 224)
    batch_size = 8
    ...
    # 输出维度应该是 (batch_size, num_classes)
