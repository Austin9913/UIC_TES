...
#动态卷积
# from Uterus_Dis_Cl.models.ODConv_main.modules.odconv import ODConv2d
#
#
...
def test_densenet_model(num_classes=3):  # 将num_classes作为函数参数
    # 设置随机种子以获得可重复的结果
    torch.manual_seed(0)

    # 创建DenseNet模型实例，指定num_classes
    model = DenseNet_new(num_classes=num_classes)

    # 创建一个随机生成的图像张量，模拟一个批量大小为1的输入
    ...
    # 打印输出，这里输出的维度应该是[1, num_classes]
    print("Output shape:", outputs.shape)

    # 可选：检查模型输出的维度
    assert outputs.size() == (1, num_classes), "Output size is incorrect."


if __name__ == "__main__":
    test_densenet_model()  # 调用测试函数