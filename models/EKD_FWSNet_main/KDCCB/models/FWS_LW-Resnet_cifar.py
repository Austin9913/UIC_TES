...
def main():
    # 配置模型参数，例如类别数量和 KL 损失权重
    num_classes = 10
    KL_w = 50

    # 构建 FWS_ResNet 模型实例
    model = FWS_resnet20(num_classes=num_classes, KL_w=KL_w)
    model.eval()  # 切换到评估模式

    # 生成一个随机输入张量，模拟一个批次的图像数据 (batch_size=4, channels=3, height=32, width=32)
    input_data = torch.randn(4, 3, 32, 32)

    # 前向传播，模型返回 output 和 KD_Loss
    output, KD_Loss = model(input_data)

    # 打印输出和损失
    print("Output:")
    for i, out in enumerate(output):
        print(f"Output branch {i + 1}: {out.shape}")

    print("\nKnowledge Distillation Loss (KD_Loss):", KD_Loss.item())
