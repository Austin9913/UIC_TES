...
__all__ = ['auc_calculate_gen_csv', 'accuracy','Single_To_Statis','Save_Metrics_Sla','Save_C_M_Sla',
...
def Single_To_Statis(df):
    ...
    for i, patient_id in enumerate(patient_ids):
        ...
        neg_preds = np.empty((len(df1), np.array(neg_pred[0]).shape[0]), dtype=float)
...
def AUC_Statis(df):
    ...
    statis = roc_auc_score(ytrue, ypred, multi_class='ovo')
    return statis
...
def AUC_Single(df):
    ...
    single = roc_auc_score(ytrue, ypred, multi_class='ovo') #问题
    return single
...
def AUC(df):
    ...
    roc_auc = dict()
    cls_len = len(person_preds[0])
    y_true = label_binarize(person_label, classes=list(range(cls_len))) # ,4,5,6,7
...
def makefile(new_file):
    if not os.path.exists(new_file):
        ...
        csv_writer.writerow(["Time", "model", "state", "epoch", "learning_rate","batch_size", "Loss",
                            "sing_auc_single", "sing_auc_statis",  # singel  # torch.Size([256, 3])
                            "sing_class_auc_statis", "sing_class1_auc_statis", "sing_class2_auc_statis",
                            "sing_acc_single", "sing_acc_statis",
                            "agg_auc_single", "agg_auc_statis",  # agg torch.Size([64, 3])
                            "agg_class0_auc_statis", "agg_class1_auc_statis", "agg_class2_auc_statis",
                            "agg_acc_single", "agg_acc_statis"])  # 3. 构建列表头

def Save_Metrics_Sla(time1, time2, arch, state, epoch, learning_rate, batch_size, loss,
                sing_auc_single, sing_auc_statis,  # singel  # torch.Size([256, 3])
                sing_class_auc_statis, sing_class1_auc_statis, sing_class2_auc_statis,
                sing_acc_single, sing_acc_statis,
                agg_auc_single, agg_auc_statis,  # agg torch.Size([64, 3])
                agg_class0_auc_statis, agg_class1_auc_statis, agg_class2_auc_statis,
                agg_acc_single, agg_acc_statis):
    makedir('/home/test/Uterus_Dis_Cl/image/{}'.format(time1))
    makefile('/home/test/Uterus_Dis_Cl/image/{}/metrics.csv'.format(time1))

    line = [time2, arch, state, epoch, learning_rate, batch_size, loss,
                sing_auc_single, sing_auc_statis,  # singel  # torch.Size([256, 3])
                sing_class_auc_statis, sing_class1_auc_statis, sing_class2_auc_statis,
                sing_acc_single, sing_acc_statis,
                agg_auc_single, agg_auc_statis,  # agg torch.Size([64, 3])
...
# 保存最好的混淆矩阵
def Save_C_M_Sla(time1, txt_path, epoch,
    ...
    f.write('\r\n' + "single：")
    f.write('\r\n' + "病例为主体：")
    f.write('\r\n' + str(train_sing_cnf_matrix_statis))
    f.write('\r\n' + "图像为主体：")
    f.write('\r\n' + str(train_sing_cnf_matrix_single))
    f.write('\r\n' + "agg：")
    f.write('\r\n' + "病例为主体：")
    f.write('\r\n' + str(train_agg_cnf_matrix_statis))
    f.write('\r\n' + "图像为主体：")
    f.write('\r\n' + str(train_agg_cnf_matrix_single))

    f.write('\r\n'+"测试集混淆矩阵：")
    f.write('\r\n' + "single：")
    f.write('\r\n' + "病例为主体：")
    f.write('\r\n' + str(test_sing_cnf_matrix_statis))
    f.write('\r\n' + "图像为主体：")
    f.write('\r\n' + str(test_sing_cnf_matrix_single))
    f.write('\r\n' + "agg：")
    f.write('\r\n' + "病例为主体：")
    f.write('\r\n' + str(test_agg_cnf_matrix_statis))
    f.write('\r\n' + "图像为主体：")
    f.write('\r\n' + str(test_agg_cnf_matrix_single))
